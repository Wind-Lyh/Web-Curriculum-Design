...
- **handleDeletePost**：处理删除帖子（管理员权限）
- **handleDeleteComment**：处理删除评论（管理员权限）
...
**作用**：用户业务逻辑实现
**方法**：
- **register**：用户注册，包含参数验证、数据校验、密码加密、积分初始化
- **login**：用户登录，包含身份验证、状态检查、登录日志记录
- **updateProfile**：更新用户个人资料，验证权限和数据有效性
- **changePassword**：修改密码，验证旧密码、检查新密码强度
- **uploadAvatar**：上传头像，处理文件上传和路径保存
- **bindEmail**：绑定邮箱，验证邮箱格式和唯一性
- **bindPhone**：绑定手机，验证手机格式和唯一性
- **getUserById**：根据ID获取用户信息，包含状态检查
- **getUserByUsername**：根据用户名获取用户信息
- **getUserByEmail**：根据邮箱获取用户信息
- **checkUsernameExists**：检查用户名是否已存在
- **checkEmailExists**：检查邮箱是否已注册
- **checkPhoneExists**：检查手机是否已绑定
- **recordLogin**：记录用户登录日志，包含IP和用户代理信息
- **getAllUsers**：分页获取所有用户，支持排序和筛选
- **banUser**：封禁用户，更新状态并记录封禁原因
- **unbanUser**：解封用户，恢复用户正常状态
- **getUserCount**：统计用户总数，支持条件统计

### UserProfileService.java（接口）
**作用**：用户个人资料扩展业务接口定义
**方法**：
- **updateSignature**：更新个人签名
- **addBrowseHistory**：添加帖子浏览记录
- **getBrowseHistory**：获取用户浏览历史（分页）
- **clearBrowseHistory**：清空用户浏览历史

### UserProfileServiceImpl.java（实现类）
**作用**：用户个人资料扩展业务实现
**方法**：
- **updateSignature**：更新用户个性签名，验证签名长度不超过100字符
- **addBrowseHistory**：添加用户浏览帖子记录，避免重复记录
- **getBrowseHistory**：分页查询用户浏览历史，验证分页参数
- **clearBrowseHistory**：清空用户的所有浏览记录
...
- **getPostWithDetails(int postId)**：获取帖子详情（包含作者信息等）

### PostServiceImpl.java（实现类）
**作用**：帖子业务逻辑实现
**方法**：
- **createPost**：创建新帖子，验证用户状态，奖励发帖积分
- **updatePost**：更新帖子，验证权限（只能修改自己的帖子）
- **deletePost**：删除帖子，支持管理员和作者删除
- **getPostById**：根据ID获取帖子，检查帖子状态
- **getPostsByUser**：获取用户发布的帖子，分页支持
- **searchPosts**：搜索帖子，支持关键词搜索和分页
- **increaseViewCount**：增加帖子浏览数
- **getPostWithDetails**：获取帖子详情（含作者信息）
...
**作用**：评论业务逻辑实现
**方法**：
- **addComment**：添加评论，验证帖子状态，奖励评论积分
- **updateComment**：更新评论，验证权限（只能修改自己的评论）
- **deleteComment**：删除评论，支持管理员和作者删除
- **getCommentsByPost**：获取帖子的所有评论
- **getCommentsByUser**：获取用户的评论，分页支持
...
### LikeServiceImpl.java（实现类）
**作用**：点赞业务逻辑实现
**方法**：
- **likePost**：点赞帖子，避免重复点赞
- **unlikePost**：取消点赞帖子
- **likeComment**：点赞评论，避免重复点赞
- **unlikeComment**：取消点赞评论
- **isLiked**：检查用户是否已点赞
- **getLikeCount**：获取目标的点赞数
...
- **getRanking**：获取积分排行榜
- **calculateLevelProgress(int userId)**：计算等级进度

### PointsServiceImpl.java（实现类）
**作用**：积分业务逻辑实现
**方法**：
- **addPoints**：增加用户积分，记录积分变动原因
- **deductPoints**：扣除用户积分，验证积分是否足够
- **getUserPoints**：获取用户当前积分
- **getUserLevel**：根据积分计算用户等级
- **exchangeGoods**：兑换虚拟道具，验证库存和积分
- **getRanking**：获取积分排行榜前N名用户
- **calculateLevelProgress**：计算用户等级进度百分比

### SearchService.java（接口）
**作用**：搜索业务接口定义
**方法**：
- **searchPosts(String keyword, int page, int pageSize)**：搜索帖子
- **searchUsers(String keyword, int page, int pageSize)**：搜索用户
- **searchComments(String keyword, int page, int pageSize)**：搜索评论
- **globalSearch(String keyword, String type, int page, int pageSize)**：全局搜索

### SearchServiceImpl.java（实现类）
**作用**：搜索业务逻辑实现
**方法**：
- **searchPosts**：根据关键词搜索帖子，调用PostDao的search方法
- **searchUsers**：根据关键词搜索用户，调用UserDao的search方法
- **searchComments**：根据关键词搜索评论，需要CommentDao支持
- **globalSearch**：全局综合搜索，支持多种搜索类型

### AdminService.java（接口）
**作用**：管理员业务接口定义
**方法**：
- **getDashboardStats()**：获取仪表板统计
- **getAllUsers(int page, int pageSize)**：获取所有用户
- **searchUsers(String keyword, String status, int page, int pageSize)**：搜索用户
- **banUser(int userId, String reason)**：封禁用户
- **unbanUser(int userId)**：解封用户
- **resetUserPassword(int userId)**：重置用户密码
- **deletePost(int postId)**：删除帖子（管理员）
- **deleteComment(int commentId)**：删除评论（管理员）
- **getAdminLogs(int page, int pageSize)**：获取管理员操作日志

### AdminServiceImpl.java（实现类）
**作用**：管理员业务逻辑实现
**方法**：
- **getDashboardStats**：获取后台管理仪表板统计数据
- **getAllUsers**：获取所有用户列表，支持分页
- **searchUsers**：按条件搜索用户，支持状态筛选
- **banUser**：封禁用户，记录封禁原因和操作日志
- **unbanUser**：解封用户，记录操作日志
- **resetUserPassword**：重置用户密码为随机字符串，记录操作日志
- **deletePost**：管理员删除帖子，记录操作日志
- **deleteComment**：管理员删除评论，记录操作日志
- **getAdminLogs**：获取管理员操作日志，支持分页
